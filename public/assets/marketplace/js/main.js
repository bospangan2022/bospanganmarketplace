...
(function ($) {
    ...
    function minicart_dropdown2() {
        $(".site-header__love2").on("click", function (i) {
            i.preventDefault();
            $("#header-love").slideToggle();
        });
        // Hide Cart on document click
        $("body").on("click", function (event) {
            var $target = $(event.target);
            if (
                !$target.parents().is(".site-love") &&
                !$target.is(".site-love")
            ) {
                $("body").find("#header-love").slideUp();
            }
        });
    }
    minicart_dropdown2();
    ...
    function load_more() {
        ...
        $(".loadMore").on("click", function (e) {
            ...
            if ($(".product-load-more .item:hidden").length == 0) {
                $(".infinitpagin").html(
                    '<div class="btn loadMore">Tidak Ada Produk</div>'
                );
            }
        });
    }
